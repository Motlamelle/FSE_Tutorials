...
def get_transactions_by_category(category_name: str):
    ...
    # First get a session
    # query the database for the category with given name
    # if category exists fetch its transactions else return an empty list
    # use the helper function to convert transactions to dicts for JSON serialization
    ...
    try:
        ...
        category = session.execute(stmt).scalars().first()
        if not category:
            logger.info(f"No category found with name '{category_name}'.")
            return []
        transactions = category.transactions
        return convert_transactions_to_dict(transactions)
    finally:        
        session.close()
